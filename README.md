# Number System Converter and Arithmetic Calculator - Complete Flowchart
...
graph TD
    A([Start]) --> B[Display program header]
    B --> C[Ask: number of inputs n]
    C --> D{n >= 3?}
    D -- No --> C
    D -- Yes --> E[Show number system menu]

    E --> F{Choice valid 1-4?}
    F -- No --> E
    F -- Yes --> G[Enter number for that base]

    G --> H{Valid for base?}
    H -- No --> G
    H -- Yes --> I[Convert to all 4 systems, Display result]

    I --> J{More inputs remaining?}
    J -- Yes --> E
    J -- No --> K[Display completion message]
    K --> L([Go to Calculator])
```
...
```
graph TD
    A([Start Calculator]) --> B[Display stored values a, b, c ...]
    B --> C{Mode 1 or 2?}
    C -- 1: Simple --> D[Select operator]
    D --> E[Build expression a op b op c ...]
    C -- 2: Custom --> F[User types expression]
    E --> G[Tokenize expression]
    F --> G
    G --> H{Tokens valid?}
    H -- No --> I[Display error, retry]
    I --> C
    H -- Yes --> J[Convert to postfix, Shunting-Yard]
    J --> K[Evaluate postfix]
    K --> L{Division by zero?}
    L -- Yes --> M[Display arithmetic error]
    L -- No --> N[Display expression with original values]
    N --> O[Display result in all 4 bases]
    M --> P([End])
    O --> P
```
...
`1010`(bin), `17`(oct), `255`(dec)  
Convert to a=10, b=15, c=255 decimal

2  
Binary + Decimal + Hexadecimal  
`1010`(bin), `255`(dec), `1A`(hex)  
Convert to a=10, b=255, c=26 decimal

3  
Octal + Decimal + Hexadecimal  
`17`(oct), `255`(dec), `1A`(hex)  
Convert to a=15, b=255, c=26 decimal

4  
Binary + Octal + Hexadecimal  
`1010`(bin), `17`(oct), `1A`(hex)  
Convert to a=10, b=15, c=26 decimal

5  
Invalid input  
`1029` as Binary  
Rejected (digits 2, 9 invalid for base 2), re-prompted
...
13  
Mixed precedence, no parentheses  
a+b*c  
3835 (multiplication happens before addition)
...
## Sample Run 1 - Converter (Binary + Octal + Decimal)

# =====================================
NUMBER SYSTEM CONVERTER

Enter the number of inputs (minimum of 3): 3

---

## INPUT NUMBER 1 (variable 'a')
...
## Sample Run 2 - Calculator (Parentheses + Precedence)

# =====================================
ARITHMETIC CALCULATOR

a = 1010 (Base 2) = 10 decimal
b = 17 (Base 8) = 15 decimal
c = 255 (Base 10) = 255 decimal
d = 1A (Base 16) = 26 decimal

Choose calculation mode:
[1] Simple operation applied to all inputs (a op b op c ...)
[2] Custom expression with parentheses (e.g. (a+b-c)*d)
Enter your choice: 2

You have variables 'a' to 'd' available.
Example: (a+b-c)*d
...
Decimal:        -5980
Binary:         -1011101011100
Octal:          -13534
Hexadecimal:    -175C

## Sample Run 3 - Calculator (Division by Zero Error Handling)

# =====================================
ARITHMETIC CALCULATOR

a = 10 (Base 10) = 10 decimal
b = 0 (Base 10) = 0 decimal
c = 5 (Base 10) = 5 decimal
